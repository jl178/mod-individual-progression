...
enum ProgressionMaps {
  // MAP_BLACKWING_LAIR = 469,MAP_BLACK
  // MAP_ZUL_GURUB = 309,
  MAP_AQ_20 = 509,
  MAP_AQ_40 = 531,
  MAP_OUTLANDS = 530,
...
enum ProgressionSettings { SETTING_PROGRESSION_STATE = 0 };
...
public:
  ...
  float vanillaPowerAdjustment, vanillaHealthAdjustment, tbcPowerAdjustment,
      ...
      previousGearTuning;
  bool enabled, questXpFix, hunterPetLevelFix, requirePreAQQuests,
      enforceGroupRules, fishingFix, simpleConfigOverride, questMoneyAtLevelCap,
  ...
  int progressionLimit, startingProgression, tbcRacesProgressionLevel,
      deathKnightProgressionLevel, deathKnightStartingProgression;
  ...
  static void ForceUpdateProgressionState(Player *player,
                                          ProgressionState newState);
  void CheckAdjustments(Player *player) const;
  void ApplyGearStatsTuning(Player *player, float &computedAdjustment,
                            ItemTemplate const *item) const;
  void ComputeGearTuning(Player *player, float &computedAdjustment,
                         ItemTemplate const *item) const;
  ...
  static void
  LoadCustomProgressionEntries(const std::string &customProgressionString);
  static void AdjustStats(Player *player, float computedAdjustment,
                          float computedHealingAdjustment);
  static float ComputeVanillaAdjustment(uint8 playerLevel,
                                        float configAdjustmentValue);
